...
export function registerCashboxRoutes(app: Express, isAuthenticated: any) {
  ...
  app.post("/api/cashbox/cutoff", isAuthenticated, async function(req: Request, res: Response) {
    try {
  ...
  app.get("/api/cashbox/transactions", isAuthenticated, async function(req: Request, res: Response) {
    try {
      const { user } = req as any;
      const userId = user.id;
      const companyId = user.companyId || user.company;
