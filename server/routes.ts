...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Esquema de validación para creación/actualización de cupones
  const couponValidationSchema = z.object({
    code: z.string().min(3, "El código debe tener al menos 3 caracteres").max(20, "El código debe tener máximo 20 caracteres"),
    discountType: z.enum(["percentage", "fixed"], {
      errorMap: () => ({ message: "El tipo de descuento debe ser 'percentage' o 'fixed'" })
    }),
    discountValue: z.number().positive("El valor del descuento debe ser positivo"),
    maxUses: z.number().int().positive("El número máximo de usos debe ser un entero positivo"),
    expiryDate: z.string().refine(val => !isNaN(Date.parse(val)), {
      message: "La fecha de expiración debe ser una fecha válida"
    }),
    description: z.string().optional(),
    minPurchaseAmount: z.number().min(0, "El monto mínimo de compra no puede ser negativo").optional(),
    active: z.boolean().optional(),
    companyId: z.string().optional()
  });
  
  // GET - Obtener todos los cupones (filtrados por compañía si aplica)
  app.get(apiRouter("/coupons"), isAuthenticated, hasRole(["dueño", "administrador"]), async (req: Request, res: Response) => {
    try {
      const user = req.user as any;
      console.log(`[GET /coupons] Usuario: ${user.firstName} ${user.lastName}, Rol: ${user.role}`);
      
      let companyId = undefined;
      
      // Si no es superAdmin, filtrar por compañía del usuario
      if (user.role !== "superAdmin") {
        companyId = user.companyId || user.company;
        console.log(`[GET /coupons] Filtrando cupones por compañía: ${companyId}`);
      }
  ...
  // GET - Obtener un cupón específico por ID
  app.get(apiRouter("/coupons/:id"), isAuthenticated, hasRole(["dueño", "administrador"]), async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const user = req.user as any;
      console.log(`[GET /coupons/${id}] Usuario: ${user.firstName} ${user.lastName}, Rol: ${user.role}`);
      
      const coupon = await storage.getCouponById(parseInt(id));
      
      if (!coupon) {
        return res.status(404).json({ error: "Cupón no encontrado" });
      }
      
      // Si no es superAdmin, verificar que el cupón pertenece a su compañía
      if (user.role !== "superAdmin") {
        const userCompanyId = user.companyId || user.company;
        if (coupon.companyId !== userCompanyId) {
          console.log(`[GET /coupons/${id}] Acceso denegado. Usuario de compañía ${userCompanyId} intentó acceder a cupón de compañía ${coupon.companyId}`);
  ...
  // GET - Verificar validez de un cupón por código
  app.get(apiRouter("/coupons/check/:code"), isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { code } = req.params;
      const user = req.user as any;
      console.log(`[GET /coupons/check/${code}] Usuario: ${user.firstName} ${user.lastName}, Rol: ${user.role}`);
      ...
      console.error(`[GET /coupons/check/:code] Error:`, error);
  ...
  app.post(apiRouter("/coupons"), isAuthenticated, hasRole(["dueño", "administrador"]), async (req: Request, res: Response) => {
    try {
      const user = req.user as any;
      console.log(`[POST /coupons] Usuario: ${user.firstName} ${user.lastName}, Rol: ${user.role}`);
      ...
      // Asegurar que solo se crean cupones para la propia compañía
      let companyId = user.companyId || user.company;
      
      // Solo superAdmin puede crear cupones para otras compañías
      ...
      // Parsear la fecha de expiración a formato Date
      couponData.expiryDate = new Date(couponData.expiryDate);
      
      console.log(`[POST /coupons] Creando cupón para compañía ${companyId}:`, couponData);
  ...
  app.put(apiRouter("/coupons/:id"), isAuthenticated, hasRole(["dueño", "administrador"]), async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const user = req.user as any;
      console.log(`[PUT /coupons/${id}] Usuario: ${user.firstName} ${user.lastName}, Rol: ${user.role}`);
      ...
      // Verificar permisos (solo puede modificar cupones de su compañía, a menos que sea superAdmin)
      if (user.role !== "superAdmin") {
        const userCompanyId = user.companyId || user.company;
        if (coupon.companyId !== userCompanyId) {
          console.log(`[PUT /coupons/${id}] Acceso denegado. Usuario de compañía ${userCompanyId} intentó modificar cupón de compañía ${coupon.companyId}`);
          return res.status(403).json({ error: "No tiene permiso para modificar este cupón" });
        }
      }
      
      // Validar datos de entrada
      const validationResult = couponValidationSchema.partial().safeParse(req.body);
      ...
      // Si se proporciona nueva fecha de expiración, convertirla a Date
      if (updateData.expiryDate) {
        updateData.expiryDate = new Date(updateData.expiryDate);
      }
      
      console.log(`[PUT /coupons/${id}] Actualizando cupón:`, updateData);
      
      const updatedCoupon = await storage.updateCoupon(parseInt(id), updateData);
      return res.json(updatedCoupon);
    } catch (error) {
      console.error(`[PUT /coupons/:id] Error:`, error);
      return res.status(500).json({ error: "Error al actualizar el cupón" });
    }
  });
  
  // DELETE - Eliminar un cupón
  app.delete(apiRouter("/coupons/:id"), isAuthenticated, hasRole(["dueño", "administrador"]), async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const user = req.user as any;
      console.log(`[DELETE /coupons/${id}] Usuario: ${user.firstName} ${user.lastName}, Rol: ${user.role}`);
      ...
      // Verificar permisos (solo puede eliminar cupones de su compañía, a menos que sea superAdmin)
      if (user.role !== "superAdmin") {
        const userCompanyId = user.companyId || user.company;
        if (coupon.companyId !== userCompanyId) {
          console.log(`[DELETE /coupons/${id}] Acceso denegado. Usuario de compañía ${userCompanyId} intentó eliminar cupón de compañía ${coupon.companyId}`);
          return res.status(403).json({ error: "No tiene permiso para eliminar este cupón" });
        }
      }
      
      // Intentar eliminar el cupón
      const result = await storage.deleteCoupon(parseInt(id));
