...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Nuevo endpoint optimizado para la edición de viajes
  app.get(apiRouter("/trips/:id/edit"), async (req: Request, res: Response) => {
    try {
      ...
      console.log(`[GET /trips/${tripId}/edit] Usuario: ${user.firstName} ${user.lastName}, Rol: ${user.role}`);
      
      // PASO 1: Obtener datos del viaje
      console.log(`[GET /trips/${tripId}/edit] Obteniendo datos del viaje...`);
      const trip = await storage.getTripWithRouteInfo(tripId);
      
      if (!trip) {
        return res.status(404).json({ error: "Viaje no encontrado" });
      }
      
      // PASO 2: Verificar permisos (igual que en el endpoint normal)
      if (user.role !== UserRole.SUPER_ADMIN && user.role !== UserRole.TICKET_OFFICE) {
        ...
        if (trip.companyId && trip.companyId.toLowerCase() !== userCompanyId.toLowerCase()) {
          return res.status(403).json({ 
            error: "No tiene permiso para editar este viaje",
            details: `El viaje pertenece a otra compañía: ${trip.companyId}`
          });
        }
      }
      
      // PASO 3: Recopilar todos los datos necesarios en paralelo
      console.log(`[GET /trips/${tripId}/edit] Obteniendo datos completos...`);
      let routeSegments = null;
      let vehicleInfo = null;
      let driverInfo = null;
      
      try {
        // Obtener datos en paralelo para maximizar rendimiento
        [routeSegments, vehicleInfo, driverInfo] = await Promise.all([
          // 1. Obtener segmentos de la ruta
          storage.getRouteWithSegments(trip.routeId),
      ...
      // PASO 4: Construir respuesta enriquecida
      const enrichedResponse = {
        // Datos básicos del viaje
        trip,
        
        // Información completa de la ruta con segmentos
        ...
        // Incluir timestamp para evitar problemas de caché
        timestamp: Date.now()
      };
