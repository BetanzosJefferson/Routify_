...
// Importar módulos para autenticación y sistema de invitaciones
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Sistema de invitaciones entre empresas para transferencias
  
  // Almacenamiento en memoria de invitaciones (para producción esto debería ser en la base de datos)
  if (!global.companyInvitations) {
    global.companyInvitations = [];
  }
  
  if (!global.companyAuthorizations) {
    global.companyAuthorizations = [];
  }
  ...
  app.post(apiRouter('/transfers/generate-invitation'), isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      // Guardar la invitación en la memoria
      global.companyInvitations.push(invitation);
  ...
  app.get(apiRouter('/transfers/validate-invitation/:token'), isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const invitation = global.companyInvitations.find((inv: any) => inv.token === token);
  ...
  app.post(apiRouter('/transfers/accept-invitation/:token'), isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      // Buscar la invitación
      const invitationIndex = global.companyInvitations.findIndex((inv: any) => inv.token === token);
      
      if (invitationIndex === -1) {
        return res.status(404).json({ error: "Invitación no encontrada" });
      }
      
      const invitation = global.companyInvitations[invitationIndex];
      ...
      invitation.targetCompanyId = userCompany.identifier;
      
      global.companyInvitations[invitationIndex] = invitation;
      
      // Añadir relación de autorización entre empresas
      global.companyAuthorizations.push({
        id: global.companyAuthorizations.length + 1,
  ...
  app.get(apiRouter('/transfers/authorized-companies'), isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const authorizations = global.companyAuthorizations.filter(
        (auth: any) => auth.sourceCompanyId === user.company && auth.isActive
      );
