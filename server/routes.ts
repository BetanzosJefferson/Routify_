...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post(apiRouter("/vehicles"), async (req: Request, res: Response) => {
    try {
      // Preparando el objeto para validación
      const vehicleData = {
        ...req.body,
        // Asegurando que los campos opcionales sean null si no están definidos
        services: req.body.services || [], // Array vacío por defecto, no null
        description: req.body.description || null,
        hasAC: typeof req.body.hasAC === 'boolean' ? req.body.hasAC : false,
        hasRecliningSeats: typeof req.body.hasRecliningSeats === 'boolean' ? req.body.hasRecliningSeats : false
      };
      
      // Validamos según el esquema de Drizzle
  ...
  app.get(apiRouter("/commissions"), async (_req: Request, res: Response) => {
    try {
  ...
  app.get(apiRouter("/commissions/:id"), async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id, 10);
  ...
  app.post(apiRouter("/commissions"), async (req: Request, res: Response) => {
    try {
      // Preparando el objeto para validación
      const commissionData = {
        ...
        percentage: typeof req.body.percentage === 'boolean' ? req.body.percentage : false
      };
  ...
  app.put(apiRouter("/commissions/:id"), async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id, 10);
      ...
      const commissionData = {
        ...
        routeId: req.body.routeId || null,
        tripId: req.body.tripId || null,
        percentage: typeof req.body.percentage === 'boolean' ? req.body.percentage : null
      };
      
      const updatedCommission = await storage.updateCommission(id, commissionData);
  ...
  app.delete(apiRouter("/commissions/:id"), async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id, 10);
