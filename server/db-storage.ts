...
export class DatabaseStorage implements IStorage {
  ...
  async getCoupons(companyId?: string): Promise<Coupon[]> {
    try {
      let query = db.select().from(schema.coupons);
      
      if (companyId) {
        console.log(`Buscando cupones para compañía: ${companyId}`);
        query = query.where(eq(schema.coupons.companyId, companyId));
      }
      
      const coupons = await query;
      console.log(`Se encontraron ${coupons.length} cupones${companyId ? ` para la compañía ${companyId}` : ''}`);
      return coupons;
    } catch (error) {
  ...
  async createCoupon(coupon: InsertCoupon): Promise<Coupon> {
    try {
      console.log(`Creando nuevo cupón con código: ${coupon.code}`);
      ...
      console.log(`Cupón creado con éxito, ID: ${newCoupon.id}`);
      return newCoupon;
    } catch (error) {
  ...
  async deleteCoupon(id: number): Promise<boolean> {
    try {
      console.log(`Eliminando cupón ID ${id}`);
      ...
      console.log(`Eliminación de cupón ${id}: ${success ? 'Éxito' : 'Fallido'}`);
      return success;
    } catch (error) {
  ...
  async getCompanies(): Promise<Company[]> {
    try {
      console.log("Obteniendo lista de todas las compañías");
      
      // Primero, obtenemos todos los usuarios con rol de dueño (owner)
      const owners = await db
        .select()
        .from(schema.users)
        .where(eq(schema.users.role, schema.UserRole.OWNER));
      
      console.log(`Encontrados ${owners.length} usuarios con rol de dueño`);
      
      // Creamos compañías a partir de los datos de los dueños
      const companies: Company[] = owners
        .filter(owner => owner.company && (owner.companyId || owner.company))
        .map(owner => ({
          id: owner.companyId || owner.company || "",
          name: owner.company || "Empresa sin nombre",
          logoUrl: owner.profilePicture || null,
      ...
      console.log(`Se procesaron ${companies.length} compañías`);
      return companies;
    } catch (error) {
  ...
  async getCompany(id: string): Promise<Company | undefined> {
    try {
      ...
      // Buscar el dueño (Owner) de la compañía para obtener la información
      const [owner] = await db
        .select()
        .from(schema.users)
        .where(
          and(
            ...
            or(
              ...
              eq(schema.users.company, id)
            )
          )
        );
      
      if (!owner) {
        console.log(`No se encontró dueño para la compañía ${id}`);
        return undefined;
      }
      
      console.log(`Encontrado dueño para compañía ${id}: ${owner.firstName} ${owner.lastName}`);
      
      // Crear objeto de compañía a partir de los datos del dueño
      const company: Company = {
        ...
        contactPhone: null,
        address: null,
        clabe: owner.clabe || null,
        accountHolder: owner.accountHolder || null,
      ...
      console.error(`Error al obtener compañía ${id}:`, error);
      return undefined;
    }
  }
  ...
  async updateCompany(id: string, companyUpdate: Partial<Company>): Promise<Company | undefined> {
    try {
      console.log(`Actualizando compañía ID ${id} con datos bancarios`);
      
      // Buscar al dueño (Owner) de la compañía
      const [owner] = await db
        .select()
        .from(schema.users)
        .where(
      ...
      // Actualizar datos del usuario dueño con los datos bancarios
      const [updatedOwner] = await db
        .update(schema.users)
        .set({
          // Actualizar solo los campos bancarios
          clabe: companyUpdate.clabe,
          accountHolder: companyUpdate.accountHolder,
          bankName: companyUpdate.bankName
        })
        .where(eq(schema.users.id, owner.id))
        .returning();
      
      if (!updatedOwner) {
        console.log(`Error al actualizar datos de dueño de la compañía ${id}`);
        return undefined;
      }
      
      console.log(`Datos bancarios de compañía ${id} actualizados correctamente`);
      
      // Crear y devolver objeto Company actualizado
      const updatedCompany: Company = {
        id: id,
        name: updatedOwner.company || "Empresa sin nombre",
        logoUrl: updatedOwner.profilePicture || null,
        contactEmail: updatedOwner.email,
        contactPhone: null,
        address: null,
        clabe: updatedOwner.clabe || null,
        accountHolder: updatedOwner.accountHolder || null,
        bankName: updatedOwner.bankName || null,
        createdAt: new Date(),
        updatedAt: new Date()
      };
      ...
      console.error(`Error al actualizar compañía ${id}:`, error);
      return undefined;
    }
  }
  ...
  async searchReservations(query: string, companyId?: string): Promise<ReservationWithDetails[]> {
    try {
      console.log(`Buscando reservaciones con consulta: "${query}"${companyId ? ` para compañía ${companyId}` : ''}`);
      
      // Primero, obtener todas las reservaciones (con o sin filtro de compañía)
      const reservations = await this.getReservations(companyId);
      ...
      // Filtrar reservaciones que coinciden con la búsqueda
      const filteredReservations = reservations.filter(reservation => {
        ...
        return (
          ...
          reservation.passengers?.some(passenger => 
            `${passenger.firstName} ${passenger.lastName}`.toLowerCase().includes(queryLower)
          )
        );
      });
      
      console.log(`Se encontraron ${filteredReservations.length} reservaciones coincidentes`);
