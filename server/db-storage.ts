...
export class DatabaseStorage implements IStorage {
  ...
  async getCompanyByEmail(email: string): Promise<{id: string, name: string, email: string} | null> {
    try {
      console.log(`[getCompanyByEmail] Buscando compañía con email: ${email}`);
      ...
      if (!company) {
        console.log(`[getCompanyByEmail] No se encontró la compañía con email: ${email}`);
        return null;
      }
      
      console.log(`[getCompanyByEmail] Compañía encontrada: ${JSON.stringify(company)}`);
      return company;
    } catch (error) {
      console.error(`[getCompanyByEmail] Error al buscar compañía por email ${email}:`, error);
      return null;
    }
  }

  // Company Link methods
  async createCompanyLink(linkData: InsertCompanyLink): Promise<CompanyLink> {
    try {
      console.log(`[createCompanyLink] Creando vínculo entre empresas: ${JSON.stringify(linkData)}`);
      ...
      console.log(`[createCompanyLink] Vínculo creado: ${JSON.stringify(companyLink)}`);
      return companyLink;
    } catch (error) {
      console.error(`[createCompanyLink] Error al crear vínculo entre empresas:`, error);
      throw error;
    }
  }

  async getCompanyLink(id: number): Promise<CompanyLink | undefined> {
    try {
      console.log(`[getCompanyLink] Buscando vínculo de empresa con ID: ${id}`);
      ...
      if (!companyLink) {
        console.log(`[getCompanyLink] No se encontró el vínculo con ID: ${id}`);
        return undefined;
      }
      
      console.log(`[getCompanyLink] Vínculo encontrado: ${JSON.stringify(companyLink)}`);
      return companyLink;
    } catch (error) {
      console.error(`[getCompanyLink] Error al buscar vínculo ID ${id}:`, error);
      return undefined;
    }
  }

  async getCompanyLinkByToken(token: string): Promise<CompanyLink | undefined> {
    try {
      console.log(`[getCompanyLinkByToken] Buscando vínculo de empresa con token`);
      
      const [companyLink] = await db
        .select()
        .from(schema.companyLinks)
        .where(eq(schema.companyLinks.oneTimeUrl, token));
      
      if (!companyLink) {
        console.log(`[getCompanyLinkByToken] No se encontró el vínculo con el token especificado`);
        return undefined;
      }
      
      console.log(`[getCompanyLinkByToken] Vínculo encontrado: ${JSON.stringify(companyLink)}`);
      return companyLink;
    } catch (error) {
      console.error(`[getCompanyLinkByToken] Error al buscar vínculo por token:`, error);
      return undefined;
    }
  }

  async updateCompanyLink(id: number, data: Partial<CompanyLink>): Promise<CompanyLink | undefined> {
    try {
      console.log(`[updateCompanyLink] Actualizando vínculo ID ${id} con datos: ${JSON.stringify(data)}`);
      ...
      if (!companyLink) {
        console.log(`[updateCompanyLink] No se pudo actualizar el vínculo ID ${id}`);
        return undefined;
      }
      
      console.log(`[updateCompanyLink] Vínculo actualizado: ${JSON.stringify(companyLink)}`);
      return companyLink;
    } catch (error) {
      console.error(`[updateCompanyLink] Error al actualizar vínculo ID ${id}:`, error);
      return undefined;
    }
  }

  async getCompanyLinks(companyId: string): Promise<CompanyLink[]> {
    try {
      console.log(`[getCompanyLinks] Buscando vínculos para la empresa ID: ${companyId}`);
      
      const companyLinks = await db
        .select()
        .from(schema.companyLinks)
        .where(or(
          eq(schema.companyLinks.sourceCompanyId, companyId),
          eq(schema.companyLinks.targetCompanyId, companyId)
        ))
        .orderBy(desc(schema.companyLinks.createdAt));
      
      console.log(`[getCompanyLinks] Se encontraron ${companyLinks.length} vínculos para la empresa`);
      return companyLinks;
    } catch (error) {
      console.error(`[getCompanyLinks] Error al buscar vínculos para empresa ID ${companyId}:`, error);
      return [];
    }
  }

  async getActiveCompanyLink(sourceCompanyId: string, targetCompanyId: string): Promise<CompanyLink | undefined> {
    try {
      console.log(`[getActiveCompanyLink] Buscando vínculo activo entre empresas ${sourceCompanyId} y ${targetCompanyId}`);
      
      const [companyLink] = await db
        .select()
        .from(schema.companyLinks)
        .where(and(
          eq(schema.companyLinks.sourceCompanyId, sourceCompanyId),
          eq(schema.companyLinks.targetCompanyId, targetCompanyId),
          eq(schema.companyLinks.status, 'active')
        ));
      
      if (!companyLink) {
        console.log(`[getActiveCompanyLink] No se encontró vínculo activo entre las empresas`);
        return undefined;
      }
      
      console.log(`[getActiveCompanyLink] Vínculo activo encontrado: ${JSON.stringify(companyLink)}`);
      return companyLink;
    } catch (error) {
      console.error(`[getActiveCompanyLink] Error al buscar vínculo activo entre empresas:`, error);
      return undefined;
    }
  }

  // Transfer Request methods
  async createTransferRequest(requestData: InsertTransferRequest): Promise<TransferRequest> {
    try {
      console.log(`[createTransferRequest] Creando solicitud de transferencia: ${JSON.stringify(requestData)}`);
      ...
      console.log(`[createTransferRequest] Solicitud creada: ${JSON.stringify(transferRequest)}`);
      return transferRequest;
    } catch (error) {
      console.error(`[createTransferRequest] Error al crear solicitud de transferencia:`, error);
      throw error;
    }
  }
  ...
  async getTransferRequest(id: number): Promise<TransferRequest | undefined> {
    try {
      console.log(`[getTransferRequest] Buscando solicitud de transferencia ID: ${id}`);
      ...
      if (!transferRequest) {
        console.log(`[getTransferRequest] No se encontró la solicitud ID: ${id}`);
        return undefined;
      }
      
      console.log(`[getTransferRequest] Solicitud encontrada: ${JSON.stringify(transferRequest)}`);
      return transferRequest;
    } catch (error) {
      console.error(`[getTransferRequest] Error al buscar solicitud ID ${id}:`, error);
      return undefined;
    }
  }

  async updateTransferRequest(id: number, data: Partial<TransferRequest>): Promise<TransferRequest | undefined> {
    try {
      console.log(`[updateTransferRequest] Actualizando solicitud ID ${id} con datos: ${JSON.stringify(data)}`);
      ...
      if (!transferRequest) {
        console.log(`[updateTransferRequest] No se pudo actualizar la solicitud ID ${id}`);
        return undefined;
      }
      
      console.log(`[updateTransferRequest] Solicitud actualizada: ${JSON.stringify(transferRequest)}`);
      return transferRequest;
    } catch (error) {
      console.error(`[updateTransferRequest] Error al actualizar solicitud ID ${id}:`, error);
      return undefined;
    }
  }

  async getSentTransferRequests(companyId: string): Promise<TransferRequest[]> {
    try {
      console.log(`[getSentTransferRequests] Buscando solicitudes enviadas por empresa ID: ${companyId}`);
      
      const transferRequests = await db
        .select()
        .from(schema.transferRequests)
        .where(eq(schema.transferRequests.sourceCompanyId, companyId))
        .orderBy(desc(schema.transferRequests.createdAt));
      
      console.log(`[getSentTransferRequests] Se encontraron ${transferRequests.length} solicitudes enviadas`);
      return transferRequests;
    } catch (error) {
      console.error(`[getSentTransferRequests] Error al buscar solicitudes enviadas por empresa ID ${companyId}:`, error);
      return [];
    }
  }

  async getReceivedTransferRequests(companyId: string): Promise<TransferRequest[]> {
    try {
      console.log(`[getReceivedTransferRequests] Buscando solicitudes recibidas por empresa ID: ${companyId}`);
      
      const transferRequests = await db
        .select()
        .from(schema.transferRequests)
        .where(eq(schema.transferRequests.targetCompanyId, companyId))
        .orderBy(desc(schema.transferRequests.createdAt));
      
      console.log(`[getReceivedTransferRequests] Se encontraron ${transferRequests.length} solicitudes recibidas`);
      return transferRequests;
    } catch (error) {
      console.error(`[getReceivedTransferRequests] Error al buscar solicitudes recibidas por empresa ID ${companyId}:`, error);
      return [];
    }
  }

  // Transfer Detail methods
  async createTransferDetail(detailData: InsertTransferDetail): Promise<TransferDetail> {
    try {
      console.log(`[createTransferDetail] Creando detalle de transferencia: ${JSON.stringify(detailData)}`);
      
      const [transferDetail] = await db
        .insert(schema.transferDetails)
        .values(detailData)
        .returning();
      
      console.log(`[createTransferDetail] Detalle creado: ${JSON.stringify(transferDetail)}`);
      return transferDetail;
    } catch (error) {
      console.error(`[createTransferDetail] Error al crear detalle de transferencia:`, error);
      throw error;
    }
  }

  async getTransferDetail(id: number): Promise<TransferDetail | undefined> {
    try {
      console.log(`[getTransferDetail] Buscando detalle de transferencia ID: ${id}`);
      
      const [transferDetail] = await db
        .select()
        .from(schema.transferDetails)
        .where(eq(schema.transferDetails.id, id));
      
      if (!transferDetail) {
        console.log(`[getTransferDetail] No se encontró el detalle ID: ${id}`);
        return undefined;
      }
      
      console.log(`[getTransferDetail] Detalle encontrado: ${JSON.stringify(transferDetail)}`);
      return transferDetail;
    } catch (error) {
      console.error(`[getTransferDetail] Error al buscar detalle ID ${id}:`, error);
      return undefined;
    }
  }

  async updateTransferDetail(id: number, data: Partial<TransferDetail>): Promise<TransferDetail | undefined> {
    try {
      console.log(`[updateTransferDetail] Actualizando detalle ID ${id} con datos: ${JSON.stringify(data)}`);
      
      const [transferDetail] = await db
        .update(schema.transferDetails)
        .set(data)
        .where(eq(schema.transferDetails.id, id))
        .returning();
      
      if (!transferDetail) {
        console.log(`[updateTransferDetail] No se pudo actualizar el detalle ID ${id}`);
        return undefined;
      }
      
      console.log(`[updateTransferDetail] Detalle actualizado: ${JSON.stringify(transferDetail)}`);
      return transferDetail;
    } catch (error) {
      console.error(`[updateTransferDetail] Error al actualizar detalle ID ${id}:`, error);
      return undefined;
    }
  }

  async getTransferDetails(transferRequestId: number): Promise<TransferDetail[]> {
    try {
      console.log(`[getTransferDetails] Buscando detalles para solicitud ID: ${transferRequestId}`);
      
      const transferDetails = await db
        .select()
        .from(schema.transferDetails)
        .where(eq(schema.transferDetails.transferRequestId, transferRequestId));
      
      console.log(`[getTransferDetails] Se encontraron ${transferDetails.length} detalles para la solicitud`);
      return transferDetails;
    } catch (error) {
      console.error(`[getTransferDetails] Error al buscar detalles para solicitud ID ${transferRequestId}:`, error);
      return [];
    }
  }

  async getTransferDetailsByReservation(reservationId: number): Promise<TransferDetail[]> {
    try {
      console.log(`[getTransferDetailsByReservation] Buscando detalles de transferencia para reservación ID: ${reservationId}`);
      
      const transferDetails = await db
        .select()
        .from(schema.transferDetails)
        .where(eq(schema.transferDetails.reservationId, reservationId));
      
      console.log(`[getTransferDetailsByReservation] Se encontraron ${transferDetails.length} detalles para la reservación`);
      return transferDetails;
    } catch (error) {
      console.error(`[getTransferDetailsByReservation] Error al buscar detalles para reservación ID ${reservationId}:`, error);
      return [];
    }
  }
}