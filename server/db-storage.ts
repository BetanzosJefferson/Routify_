...
export class DatabaseStorage implements IStorage {
  ...
  async getTransaccionesByCompany(companyId: string): Promise<schema.Transaccion[]> {
    try {
      console.log(`[getTransaccionesByCompany] Buscando transacciones para la compañía ${companyId}`);
      
      // Primero verificamos si hay transacciones en la tabla
      const allTransactions = await db.select().from(schema.transacciones);
      console.log(`[getTransaccionesByCompany] Total de transacciones en la base de datos: ${allTransactions.length}`);
      ...
      console.log(`[getTransaccionesByCompany] Resultados del join: ${result.length}`);
      
      if (result.length === 0) {
        // Si no hay resultados, verificamos si el problema es la condición del join
        const usersInCompany = await db.select().from(schema.users).where(eq(schema.users.company_id, companyId));
        console.log(`[getTransaccionesByCompany] Usuarios en la compañía ${companyId}: ${usersInCompany.length}`);
        
        if (usersInCompany.length > 0) {
          const userIds = usersInCompany.map(u => u.id);
          console.log(`[getTransaccionesByCompany] IDs de usuarios en la compañía: ${userIds.join(', ')}`);
          
          // Verificar si hay transacciones para estos usuarios
          const transaccionesUsuarios = await db.select()
            .from(schema.transacciones)
            .where(inArray(schema.transacciones.usuario_id, userIds));
            
          console.log(`[getTransaccionesByCompany] Transacciones para usuarios de la compañía: ${transaccionesUsuarios.length}`);
        }
      }
