...
// ESQUEMA PARA VINCULACIÓN DE EMPRESAS
export const companyLinks = pgTable("company_links", {
  id: serial("id").primaryKey(),
  sourceCompanyId: text("source_company_id").notNull().references(() => companies.identifier),
  targetCompanyId: text("target_company_id").notNull().references(() => companies.identifier),
  status: text("status").notNull().default('pending'), // pending, active, rejected
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
  oneTimeUrl: text("one_time_url").notNull(),
  oneTimeUrlExpiry: timestamp("one_time_url_expiry").notNull(),
  lastUsed: timestamp("last_used"),
});

export const insertCompanyLinkSchema = createInsertSchema(companyLinks).omit({ 
  id: true, 
  updatedAt: true,
  lastUsed: true
});
export type InsertCompanyLink = z.infer<typeof insertCompanyLinkSchema>;
export type CompanyLink = typeof companyLinks.$inferSelect;

// ESQUEMA PARA SOLICITUDES DE TRANSFERENCIA DE PASAJEROS
...
// ESQUEMA PARA DETALLES DE TRANSFERENCIA (PASAJEROS INDIVIDUALES)
export const transferDetails = pgTable("transfer_details", {
  ...
  sourceTripId: integer("source_trip_id").notNull().references(() => trips.id),
  targetTripId: integer("target_trip_id").references(() => trips.id), // Puede ser nulo hasta que se asigne
  status: text("status").notNull().default('pending'), // pending, transferred, incompatible, cancelled
  notes: text("notes"),
  originalPrice: doublePrecision("original_price"),
  originalCreatedBy: integer("original_created_by").references(() => users.id),
  originalReservationCode: text("original_reservation_code"),
});

export const insertTransferDetailSchema = createInsertSchema(transferDetails).omit({ 
  id: true,
  targetTripId: true,
  status: true,
  notes: true
});
export type InsertTransferDetail = z.infer<typeof insertTransferDetailSchema>;
export type TransferDetail = typeof transferDetails.$inferSelect;

// RELACIONES PARA VINCULACIÓN DE EMPRESAS Y TRANSFERENCIAS
export const companyLinkRelations = relations(companyLinks, ({ one }) => ({
  sourceCompany: one(companies, {
    fields: [companyLinks.sourceCompanyId],
    references: [companies.identifier]
  }),
  targetCompany: one(companies, {
    fields: [companyLinks.targetCompanyId],
    references: [companies.identifier]
  })
}));

export const transferRequestRelations = relations(transferRequests, ({ one, many }) => ({
  sourceCompany: one(companies, {
  ...
  details: many(transferDetails)
}));

export const transferDetailRelations = relations(transferDetails, ({ one }) => ({
  transferRequest: one(transferRequests, {
    fields: [transferDetails.transferRequestId],
    references: [transferRequests.id]
  }),
  reservation: one(reservations, {
    fields: [transferDetails.reservationId],
    references: [reservations.id]
  }),
  sourceTrip: one(trips, {
    fields: [transferDetails.sourceTripId],
    references: [trips.id]
  }),
  targetTrip: one(trips, {
    fields: [transferDetails.targetTripId],
    references: [trips.id]
  }),
  originalCreator: one(users, {
    fields: [transferDetails.originalCreatedBy],
    references: [users.id]
  })
}));
