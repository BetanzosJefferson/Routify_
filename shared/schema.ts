...
// TRIP TRANSFERS SCHEMA
export const tripTransfers = pgTable('trip_transfers', {
  id: serial('id').primaryKey(),
  ...
  targetTripId: integer('target_trip_id').notNull().references(() => trips.id),
  reason: text('reason'),
  ...
  routeMapping: json('route_mapping').$type<{
...
// RESERVATION TRANSFERS SCHEMA
export const reservationTransfers = pgTable('reservation_transfers', {
  id: serial('id').primaryKey(),
  reservationId: integer('reservation_id').notNull().references(() => reservations.id),
  transferId: integer('transfer_id').notNull().references(() => tripTransfers.id),
  originalOrigin: text('original_origin').notNull(),
  originalDestination: text('original_destination').notNull(),
  newOrigin: text('new_origin').notNull(),
  newDestination: text('new_destination').notNull(),
  transferredAt: timestamp('transferred_at').notNull().defaultNow(),
});

// Insert Schemas
export const insertTripTransferSchema = createInsertSchema(tripTransfers).omit({ 
  id: true, 
  requestedAt: true,
  respondedAt: true,
  approvedBy: true,
...
export const insertReservationTransferSchema = createInsertSchema(reservationTransfers).omit({ 
  id: true,
  transferredAt: true,
});

// Types
export type InsertTripTransfer = z.infer<typeof insertTripTransferSchema>;
export type TripTransfer = typeof tripTransfers.$inferSelect;

export type InsertReservationTransfer = z.infer<typeof insertReservationTransferSchema>;
export type ReservationTransfer = typeof reservationTransfers.$inferSelect;

// Relations
export const tripTransfersRelations = relations(tripTransfers, ({ one, many }) => ({
  sourceCompany: one(companies, { 
    fields: [tripTransfers.sourceCompanyId], 
    references: [companies.identifier] 
  }),
  targetCompany: one(companies, { 
    fields: [tripTransfers.targetCompanyId], 
    references: [companies.identifier] 
  }),
  sourceTrip: one(trips, { 
    fields: [tripTransfers.sourceTripId], 
    references: [trips.id] 
  }),
  targetTrip: one(trips, { 
    fields: [tripTransfers.targetTripId], 
    references: [trips.id] 
  }),
  reservations: many(reservationTransfers),
  requestedUser: one(users, { 
    fields: [tripTransfers.createdBy], 
    references: [users.id] 
  }),
  approvedUser: one(users, { 
    fields: [tripTransfers.approvedBy], 
    references: [users.id] 
  }),
}));

export const reservationTransfersRelations = relations(reservationTransfers, ({ one }) => ({
  reservation: one(reservations, { 
    fields: [reservationTransfers.reservationId], 
    references: [reservations.id] 
  }),
  transfer: one(tripTransfers, { 
    fields: [reservationTransfers.transferId], 
    references: [tripTransfers.id] 
  }),
}));
